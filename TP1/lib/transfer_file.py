...
class ConnectionManager:
    def __init__(self, conection_function, args):
        ...
        self.join_handle = threading.Thread(
            target=conection_function, args=(self.channel,) + args
        )
...
# selective
def send_file(
    message_receiver: Channel, options: Options, sock: socket, seq_num, file
) -> ConnectionStatus:
...
def receive_file(
    ...
    while time.time() - last_usefull_package_time < RECEIVE_TIMEOUT:
        msg = message_receiver.get(
            RECEIVE_TIMEOUT - (time.time() - last_usefull_package_time)
        )
